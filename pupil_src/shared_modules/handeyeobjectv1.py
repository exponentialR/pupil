...
class HandEyeObject(Plugin):
    ...
    def recent_events(self, events):
        ...
        text_to_say = '{}and{}'.format(action, self.folder_number)
        with self.hands.Hands(min_tracking_confidence=0.7, min_detection_confidence=0.6) as mediapipe_hands:
            if self.current_action < len(self.f_content['actions']) and self.folder_number < folder:

                image = events['frame'].img
                text_to_display = 'current action : {} current folder : {}, frame number : {} '.format(action,
                                                                                                       self.folder_number,
                                                                                                       self.frame_num)
